...
class Asynchronizer(object):
    """
    An Asynchronizer executes object methods in a background process.  Its compute
    method accepts an object method, specified as object.method, a tuple of args
    and a dict of keyword args.  Subsequent calls to compute with the same parameters
    args will return None if the worker process is still running and otherwise
    will return the result.  The intended application is to allow a GUI to run a
    cpu-bound computation while still remaining responsive.  The GUI can poll the
    Asynchronizer on a timer, e.g. with window.after in Tkinter, until the result
    has been computed.

    Instantiating an Asynchronizer creates a socket and launches a listener
    process listening on the socket for requests of things to compute.  The
    listener process inherits an initialized copy of the Asynchronizer object,
    and uses its methods. Methods whose name begins with an underscore, are
    meant to be called only from the listener process or, in the case of _task,
    from a worker process created by the listener.  These methods should not be
    called from the process which instantiates the Asynchronizer.

    Caveat: an Asynchronizer only works with objects and arguments that can be
    pickled, as it needs to send these things through a socket.
    """
    ...
    def _listen(self):
        ...
        while 1:
            # A side effect of calling this is that it cleans up zombie processes.
    ...
    def _task(self, question):
        """
        Workers run this method to compute an answer.
        """
    ...
    def ask(self, question, argument=None):
        """
        Send a question to the listener and return the answer as a string.  A
        question is a one-word string.  The optional argument is an arbitrary
        byte sequence with no encoding.  Neither the question nor the argument
        should contain the eol marker (by default, carriage return followed by
        newline).

        Returns the answer to the question as a pickle.
        
        This method is not intended for humans.
        """
        if not self.socket:
    ...
    def compute(self, method, args=tuple(), kwargs={}):
        ...
        snappy Manifold object M.

        The result will be compute asynchronously by a worker process and then cached.
        
        The return value will be None if the worker has not finished.
        """
